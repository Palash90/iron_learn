...
pub fn add_i8() {
    ...
    let result = GpuTensor::new(vec![1, 2], vec![4i8, 6i8]).unwrap();
...
pub fn add_i16() {
    ...
    let m1 = GpuTensor::new(vec![1], vec![1i16]).unwrap();
    let m2 = GpuTensor::new(vec![1], vec![1i16]).unwrap();
    let result = GpuTensor::new(vec![1], vec![2i16]).unwrap();

    assert_eq!(result, (m1 + m2).unwrap());

    let m1 = GpuTensor::new(vec![1, 2], vec![1i16, 2i16]).unwrap();
    let m2 = GpuTensor::new(vec![1, 2], vec![3i16, 4i16]).unwrap();
    let result = GpuTensor::new(vec![1, 2], vec![4i16, 6i16]).unwrap();
...
pub fn add_i32() {
    ...
    let m1 = GpuTensor::new(vec![1], vec![1]).unwrap();
    let m2 = GpuTensor::new(vec![1], vec![3]).unwrap();
    let result = GpuTensor::new(vec![1], vec![4]).unwrap();

    assert_eq!(result, (m1 + m2).unwrap());

    let m1 = GpuTensor::new(vec![1, 2], vec![1, 2]).unwrap();
    let m2 = GpuTensor::new(vec![1, 2], vec![3, 4]).unwrap();
    let result = GpuTensor::new(vec![1, 2], vec![4, 6]).unwrap();
...
pub fn mul_i32() {
    ...
    let m1 = GpuTensor::new(vec![1, 1], vec![1]).unwrap();
    let m2 = GpuTensor::new(vec![1, 1], vec![3]).unwrap();

    assert_eq!(3, (m1 * m2).unwrap().get_data()[0]);

    let m1 = GpuTensor::new(vec![2, 2], vec![1, 2, 3, 4]).unwrap();
    let m2 = GpuTensor::new(vec![2, 2], vec![5, 6, 7, 8]).unwrap();
    let result = GpuTensor::new(vec![2, 2], vec![19, 22, 43, 50]).unwrap();
...
pub fn mul_2_cols() {
    ...
    let m1 = GpuTensor::new(vec![1, 2], vec![1, 2]).unwrap();
    let m2 = GpuTensor::new(vec![1, 2], vec![3, 4]).unwrap().t().unwrap();
...
pub fn add_with_shape_mismatch() {
    ...
    let m1 = GpuTensor::new(vec![1, 3], vec![1, 2, 5]).unwrap();
    let m2 = GpuTensor::new(vec![1, 2], vec![3, 4]).unwrap();
    let result = GpuTensor::new(vec![1, 2], vec![4, 6]).unwrap();
...
pub fn mul_i32_panics() {
    ...
    let m1 = GpuTensor::new(vec![2, 1], vec![1, 2]).unwrap();
    let m2 = GpuTensor::new(vec![2, 2], vec![5, 6, 7, 8]).unwrap();
    let result = GpuTensor::new(vec![2, 2], vec![19, 22, 43, 50]).unwrap();
...
pub fn mul_i32_reverse() {
    ...
    let m1 = GpuTensor::new(vec![1, 2], vec![1, 2]).unwrap();
    let m2 = GpuTensor::new(vec![2, 2], vec![5, 6, 7, 8]).unwrap();
    let result = GpuTensor::new(vec![1, 2], vec![19, 22]).unwrap();
...
pub fn mul_random_d() {
    ...
    .unwrap();
    let m2 = GpuTensor::new(vec![2, 3], vec![8, 30, 31, 11, 22, 11]).unwrap();
    let result = GpuTensor::new(
        vec![6, 3],
        vec![
