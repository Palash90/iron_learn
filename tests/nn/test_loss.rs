...
mod loss_tests {
    ...
    use iron_learn::nn::loss_functions::get_loss_function;
    ...
    fn test_mse_loss_and_prime() {
        ...
        let mse = get_loss_function(&LossFunctionType::MeanSquaredError);

        // Loss: mean((pred - actual)^2) = mean([0.25,0.25]) = 0.25
        let res = mse.0(&y_true, &y_pred).expect("MSE loss failed");
        ...
        let grad = mse.1(&y_true, &y_pred).expect("MSE loss_prime failed");
    ...
    fn test_binary_cross_entropy_loss_and_prime() {
        ...
        let bce = get_loss_function(&LossFunctionType::BinaryCrossEntropy);
        ...
        let grad = bce.1(&y_true, &y_pred).expect("BCE loss_prime failed");
    ...
    fn test_cce_loss_and_prime_gpu() {
        let cce = get_loss_function(&LossFunctionType::CategoricalCrossEntropy);
        ...
        let grad = cce.1(&y_true, &y_pred).expect("CCE loss_prime failed");
        let g = grad.get_data();
    ...
    fn test_cce_numerical_stability() {
        ...
        let res = cce.0(&y_true, &y_pred).expect("CCE stability test failed");
